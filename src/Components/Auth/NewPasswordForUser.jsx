...
export default function NewPasswordForUser() {
  const { mailId } = useParams();
  console.log("Decoded mailId:", atob(mailId));
  const navigate = useNavigate();
  const dispatch = useDispatch();
  //   const otpinfor = JSON.parse(localStorage.getItem("TempEmail"));
  const otpinfor = atob(mailId);
      ...
      <LoginLayout>
        <div className="login-box" id="login">
          <div className="text-center mb-5">
            <h2 className="text-center fw-bold letter-spacing" style={{ color: "#62a6dc" }}>
              Create Password
            </h2>
